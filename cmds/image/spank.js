...
module.exports = class SpankCommand extends Command { //MeowCommand
	constructor(client) {
		super(client, {
			name: 'spank', // meow
			aliases: [], // kitty-cat
			group: 'image', // fun
			memberName: 'spank', // filename (meow)
			description: 'Spank Image Manipulation', // desc /Replies with a meow, kitty cat.
      guildOnly: true,
//      hidden: true, // false
//      ownerOnly: true,
...
//	run(message, { text }) {    //with args
    ...
    let IMG = await canvacord.Canvas.spank(AV2, AV)
    let ATTCH = new MessageAttachment(IMG, `spank.png`)
