...
module.exports = class WantedCommand extends Command { //MeowCommand
	constructor(client) {
		super(client, {
			name: 'wanted', // meow
			aliases: [], // kitty-cat
			group: 'image', // fun
			memberName: 'wanted', // filename (meow)
			description: 'Wanted Image Manipulation', // desc /Replies with a meow, kitty cat.
      guildOnly: true,
//      hidden: true, // false
//      ownerOnly: true,
...
//	run(message, { text }) {    //with args
    ...
    let IMG = await canvacord.Canvas.wanted(AV)
    let ATTCH = new MessageAttachment(IMG, `wanted.png`)
