...
module.exports = class WastedCommand extends Command { //MeowCommand
	constructor(client) {
		super(client, {
			name: 'wasted', // meow
			aliases: [], // kitty-cat
			group: 'image', // fun
			memberName: 'wasted', // filename (meow)
			description: 'Wasted Image Manipulation', // desc /Replies with a meow, kitty cat.
      guildOnly: true,
//      hidden: true, // false
//      ownerOnly: true,
...
//	run(message, { text }) {    //with args
    ...
    let IMG = await canvacord.Canvas.wasted(AV)
    let ATTCH = new MessageAttachment(IMG, `wasted.png`)
